...
export default function CategorySection() {
  const { MundoAbierto } = useParams();
  const [game, setGame] = useState([]);

  const filtros = game.filter((x) => x.esunjuego == "si");

  const filterMundoAbierto = filtros.filter((x) => {
    if (x.categoria1 == MundoAbierto) return true;
    if (x.categoria2 == MundoAbierto) return true;
    if (x.categoria3 == MundoAbierto) return true;
  });
  ...
  return (
    <>
      <Container>
        ...
        >
          Juego de {MundoAbierto} mas popular:
        </h5>
        <Slider />
      </Container>
